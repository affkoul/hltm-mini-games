...
// Add this near your top-level setup, AFTER you define `winner_container`
(function addPlayAgain() {
    if (!winner_container) return;
    const btn = document.createElement('button');
    btn.textContent = 'Play again';
    btn.style.cssText = 'margin-top:16px;padding:10px 18px;border:0;border-radius:10px;background:#1dc525;color:#fff;font-weight:700;cursor:pointer;';
    winner_container.appendChild(btn);
    btn.addEventListener('click', () => {
        document.body.classList.remove('playing'); // turn off floating dice mode
        location.reload();                          // quick reset back to menu
    });
})();
            ...
            if (winner === 1) {
                ...
                Object.entries(global_object).forEach(([key, value]) => {
                    if (global_object[key].win_position === 0) {
                        ...
                        LudoPortal.award(1, 'Ludo: token entered board');
                    }
                });
                ...
                mover_in.pause();
                dice.disabled = true;
                document.body.classList.remove('playing');  // hide floating dice/current-player in winner screen
...
// (Put as very last line in ludo-script.js)
