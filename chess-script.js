// --- PlatformSDK session helpers (Chess) ---
let platformSessionStarted = false;

function platformEnsureStart() {
    if (!platformSessionStarted && window.PlatformSDK) {
        // Defensive: close any stray prior session (no-op if none)
        PlatformSDK.endGame();
        PlatformSDK.startGame('CHESS');   // unique id for this game
        platformSessionStarted = true;
    }
}

function platformEnd(reason = '') {
...
class ChessGame {
    constructor() {
        ...
        this.moveSound = new Audio('chess_assets/move-self.mp3');
        this.captureSound = new Audio('chess_assets/capture.mp3');
    ...
    makeMove(fromRow, fromCol, toRow, toCol) {
        ...
        if (targetPiece) {
            ...
            if (targetPiece.type === 'king') {
                ...
                if (targetPiece && window.PlatformSDK) {
                    PlatformSDK.addPoints(1, `Chess: captured ${targetPiece.type}`);
                }
                return;
            }
        } else {
    ...
    endGameWithKingCapture(timeTaken) {
        ...
        if (winner === 'You') {
            ...
            if (window.PlatformSDK) {
                PlatformSDK.addPoints(1, 'Chess: king captured');
                PlatformSDK.endGame();
            }
        }
    ...
    showWinPopup(winner, timeTaken, totalWins) {
        ...
        // Remove any previous popup to avoid duplicate IDs/listeners
        document.querySelectorAll('.win-popup').forEach(n => n.remove());
  ...
  `;
        ...
        // Bind from the popup we just appended
  ...
  `;
    ...
    checkGameStatus() {
        if (this.isCheckmate(this.currentPlayer)) {
            ...
            if (winner === 'You') {
                ...
                localStorage.setItem('totalWins', totalWins.toString());
                if (window.PlatformSDK) {
                    PlatformSDK.addPoints(1, 'Chess: checkmate');
                    PlatformSDK.endGame();
                }
            }
            this.showWinPopup(winner, timeTaken, totalWins);
    ...
    endGame(message) {
        ...
        if (window.PlatformSDK) {
            PlatformSDK.addPoints(1, 'Chess: draw');
            PlatformSDK.endGame();
        }
    ...
    updateStatusLine() {
        ...
        const L = {
            ...
            fr: { white: 'Aux blancs de jouer', black: 'Aux noirs de jouer' },
            zh: { white: '轮到白方走棋', black: '轮到黑方走棋' }
        };

        const pack = L[lang] || L.en;
        el.textContent = pack[this.currentPlayer] || '';
    }
    ...
    newGame() {
        // Close prior session and start a fresh one
        platformEnd();
        platformEnsureStart();
...
window.addEventListener('storage', (e) => {
    if (e.key === 'portal.lang' && window.game && typeof window.game.updateStatusLine === 'function') {
        // The <html lang> is set in your head script. Just refresh the text:
        window.game.updateStatusLine();
    }
});
